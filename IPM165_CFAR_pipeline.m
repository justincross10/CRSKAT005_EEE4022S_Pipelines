...
% CFAR parameters
num_guard = 250;      % Number of guard frames on each side of the Cell Under Test (CUT)
num_ref   = 250;      % Number of reference frames on each side used to estimate noise
alpha     = 10;        % Scaling factor for noise estimate
...
for r = 1:rows
    for c = 1:cols
        ...
        if detections(r,c) == 1 && visited(r,c) == 0
            ...
            freq_range = [freq_axis_cropped(min(cluster(:,1))), freq_axis_cropped(max(cluster(:,1)))];
            time_range = [time_axis(min(cluster(:,2))), time_axis(max(cluster(:,2)))];
        end
    end
end
...
ylabel('Frequency (Hz)');
xlim([t_start t_end]);
title('Detection Map with Valid Clusters for CFAR');
hold on;
